...
func (t *TreeV6) walkTree(left, right uint, walkFn func([]uint8, int) bool, depth int) error {
	if left == 0 && right == 0 {
		return errStop
	}
	var err error
...
func (t *TreeV6) Walk(walkFn func([]uint8, int) bool) error {
	root := &t.nodes[1]
...
func (t *TreeV6) FindSubnetTags(address patricia.IPv6Address, inclusive bool) (bool, []uint8, error) {
	root := &t.nodes[1]
	var found bool
	...
	for {
		...
		if matchCount == address.Length {
			...
			err := t.walkTree(node.Left, node.Right, func(types []uint8, i int) bool {
