...
func (t *TreeV4) walkTree(left, right uint, walkFn func([]float32, int) bool, depth int) error {
	if left == 0 && right == 0 {
		return errStop
	}
	var err error
...
func (t *TreeV4) Walk(walkFn func([]float32, int) bool) error {
	root := &t.nodes[1]
...
func (t *TreeV4) FindSubnetTags(address patricia.IPv4Address, inclusive bool) (bool, []float32, error) {
	root := &t.nodes[1]
	var found bool
	...
	for {
		...
		if matchCount == address.Length {
			...
			err := t.walkTree(node.Left, node.Right, func(types []float32, i int) bool {
