...
func (t *TreeV6) walkTree(left, right uint, walkFn func([]int, int) bool, depth int) error {
	if left == 0 && right == 0 {
		return errStop
	}
	var err error
...
func (t *TreeV6) Walk(walkFn func([]int, int) bool) error {
	root := &t.nodes[1]
...
func (t *TreeV6) FindSubnetTags(address patricia.IPv6Address, inclusive bool) (bool, []int, error) {
	root := &t.nodes[1]
	var found bool
	...
	for {
		...
		if matchCount == address.Length {
			...
			err := t.walkTree(node.Left, node.Right, func(types []int, i int) bool {
