...
func (t *TreeV4) walkTree(left, right uint, walkFn func([]GeneratedType, int) bool, depth int) error {
	if left == 0 && right == 0 {
		return errStop
	}
	var err error
...
func (t *TreeV4) Walk(walkFn func([]GeneratedType, int) bool) error {
	root := &t.nodes[1]
...
func (t *TreeV4) FindSubnetTags(address patricia.IPv4Address, inclusive bool) (bool, []GeneratedType, error) {
	root := &t.nodes[1]
	var found bool
	...
	for {
		...
		if matchCount == address.Length {
			// exact match - we're done
			var results []GeneratedType
			if inclusive {
				results = t.tagsForNode(retTagIndex)
			}
			err := t.walkTree(node.Left, node.Right, func(types []GeneratedType, i int) bool {
